...
SUCCESS_MSG = "repl_dmdunit_promo_curr table load was successful"

CLUSTER_NAME = "repl-dl-ephe-repl-dmdunit-promo-curr-log"
REPL_JAR_PATH = "gs://dca-repl-airflow-dag/JARS/"
REPL_FILE_PATH = "gs://dca-repl-airflow-dag/files/"

DAG_NAME = "repl_dmdunit_promo_curr_dag"
DAG_DESC = "REPL DMDUNIT PROMO CURR spark load job"
APP_NAME = "REPL_DMDUNIT_PROMO_CURR"
...
repl_dmdunit_promo_curr_load_dag = DAG(
    DAG_NAME,
...
SIGNAL_FILE_PATTERN = "Signal_GRS_DMDUNIT_PROMO_USWMTNIGHTBATCH*.txt"
POLL_PERIOD = "5"
WAIT_PERIOD = "120"

repl_dmdunit_promo_curr_common_event_poll = SSHOperator(
    ...
    dag=repl_dmdunit_promo_curr_load_dag,
    retries=3,
    trigger_rule='all_success'
)

# **************** Oldest File check Task started ******************
repl_dmdunit_promo_curr_oldest_file_check = SSHOperator(
...
SCHEMA_NAME = Variable.get("repl_dmdunit_promo_curr_load_dag_var", deserialize_json=True)["schema_name"]
TABLE_NAME = Variable.get("repl_dmdunit_promo_curr_load_dag_var", deserialize_json=True)["table_name"]
LOG_TABLE_NAME = Variable.get("repl_dmdunit_promo_curr_load_dag_var", deserialize_json=True)["log_table_name"]
# SCHEMA_NAME = "ww_repl_dl_secure"
# TABLE_NAME = "repl_dmdunit_promo_curr"
# LOG_TABLE_NAME = "repl_dmdunit_promo_log"

TGT_TABLE = SCHEMA_NAME + "." + TABLE_NAME

OLDEST_FILE_UPSERT = FILE_PATH + "/GRS_DMDUNIT_PROMO_UPSERT_USWMTNIGHTBATCH" + DATETIME + "_JWUSRB2U*"
OLDEST_FILE_DELETE = FILE_PATH + "/GRS_DMDUNIT_PROMO_DELETE_USWMTNIGHTBATCH" + DATETIME + "_JWUSRB2U*"
...
TABLE_PATH_DELETE = TABLE_NAME + "/delete"
LOCATION_UPSERT = LANDING_HDFS_PATH + "/incremental/us/" + SCHEMA_NAME + "/" + TABLE_PATH_UPSERT + "/src_rcv_dt"
LOCATION_DELETE = LANDING_HDFS_PATH + "/incremental/us/" + SCHEMA_NAME + "/" + TABLE_PATH_DELETE + "/src_rcv_dt"
...
repl_dmdunit_promo_curr_upsert_file_move_to_hdfs = SSHOperator(
    task_id="upsert_file_move_to_hdfs",
    ssh_hook=sshHook,
    conn_timeout=600, cmd_timeout=600,
    command="sh /u/appaorta/aorta/scripts/nfs_cp_resources/Bin_File_GCS_Put.sh  '" + OLDEST_FILE_UPSERT + "' " + LANDING_HDFS_PATH + " " + LOAD_TYPE + " " + COUNTRY_CODE + " " + SCHEMA_NAME + " " + TABLE_PATH_UPSERT + " " + SRCRCVDT,
    dag=repl_dmdunit_promo_curr_load_dag,
    retries=3,
    trigger_rule='all_success'
)

repl_dmdunit_promo_curr_delete_file_move_to_hdfs = SSHOperator(
    task_id="delete_file_move_to_hdfs",
    ssh_hook=sshHook,
    conn_timeout=600, cmd_timeout=600,
    command="sh /u/appaorta/aorta/scripts/nfs_cp_resources/Bin_File_GCS_Put.sh  '" + OLDEST_FILE_DELETE + "' " + LANDING_HDFS_PATH + " " + LOAD_TYPE + " " + COUNTRY_CODE + " " + SCHEMA_NAME + " " + TABLE_PATH_DELETE + " " + SRCRCVDT,
    dag=repl_dmdunit_promo_curr_load_dag,
    retries=3,
    trigger_rule='all_success'
)
...
create_bfdms_dpaas_cluster = BFDMSDataprocCreateClusterOperator(
    ...
    cluster_name=CLUSTER_NAME, dag=repl_dmdunit_promo_curr_load_dag,
...
def create_spark_job(WORKFLOW_NAME):
    return {
        ...
        "spark_job": {
            ...
            "args": ["workflow=" + WORKFLOW_NAME, "location_upsert=" + LOCATION_UPSERT,
                     "location_delete=" + LOCATION_DELETE, "src_rcv_dt=" + SRCRCVDT, "Schema=" + SCHEMA_NAME,
                     "Table=" + TABLE_NAME, "runmode=global", "enableservices=Dataquality,Audit", "runtype=standalone",
                     "run_dq=true", "env=" + ENV, "LogTable=" + LOG_TABLE_NAME, "CatlgTable=" + TGT_TABLE,
                     "TgtTable=" + TGT_TABLE
                     ],
...
repl_dmdunit_promo_curr_spark_load_job = DataprocSubmitJobOperator(
    task_id='repl_dmdunit_promo_curr_spark_load_job', job=create_spark_job("REPL_DMDUNIT_PROMO"),
    gcp_conn_id=CONN_ID, dag=repl_dmdunit_promo_curr_load_dag, trigger_rule='all_success')
...
# sh /edge_data/code/svcmdsedat/availability/resources/scripts/GCP_Archive_SourceFile.sh &LANDING_HDFS_PATH# &ARCHIVE_HDFS_PATH# &LOAD_TYPE# &COUNTRY_CODE# &SCHEMA_NAME# &TABLE_NAME# &SRCRCVDT#
repl_dmdunit_promo_curr_upsert_file_archive_in_hdfs = SSHOperator(
    task_id="upsert_file_archive_in_hdfs",
...
repl_dmdunit_promo_curr_delete_file_archive_in_hdfs = SSHOperator(
    task_id="delete_file_archive_in_hdfs",
    ssh_hook=edgenode_sshHook,
    conn_timeout=600, cmd_timeout=600,
    command="sh /edge_data/code/svcmdsedat/availability/resources/scripts/GCP_Archive_SourceFile.sh  " + LANDING_HDFS_PATH + " " + ARCHIVE_HDFS_PATH + " " + LOAD_TYPE + " " + COUNTRY_CODE + " " + SCHEMA_NAME + " " + TABLE_PATH_DELETE + " " + SRCRCVDT,
    dag=repl_dmdunit_promo_curr_load_dag,
    trigger_rule='all_success'
)

# command_delete = "gcloud auth activate-service-account --key-file=/usr/local/airflow/dags_mount/dags/mdsereplprd-prod-sa1-key.json; gcloud auth list; gsutil cat gs://dca-repl-airflow-dag/files/GCP_Archive_SourceFile.sh  | sh -s " + LANDING_HDFS_PATH + " " + ARCHIVE_HDFS_PATH + " " + LOAD_TYPE + " " + COUNTRY_CODE + " " + SCHEMA_NAME + " " + TABLE_PATH_DELETE + " " + SRCRCVDT
# repl_dmdunit_promo_curr_delete_file_archive_in_hdfs = BashOperator(
#     task_id="delete_file_archive_in_hdfs",
#     bash_command=command_delete,
#     dag=repl_dmdunit_promo_curr_load_dag,
#     trigger_rule='all_success'
# )

# **************** Delete Oldest file from NFS mount Task started ******************
repl_dmdunit_promo_curr_delete_oldest_file_upsert = SSHOperator(
    task_id="delete_oldest_file_upsert",
    ssh_hook=sshHook,
    conn_timeout=600, cmd_timeout=600,
...
repl_dmdunit_promo_curr_delete_oldest_file_delete = SSHOperator(
    task_id="delete_oldest_file_delete",
    ssh_hook=sshHook,
    conn_timeout=600, cmd_timeout=600,
...
repl_dmdunit_promo_curr_delete_signal_file = SSHOperator(
...
repl_dmdunit_promo_curr_common_event_poll >> repl_dmdunit_promo_curr_oldest_file_check >> assign_variable >> repl_dmdunit_promo_curr_upsert_file_move_to_hdfs >> repl_dmdunit_promo_curr_delete_file_move_to_hdfs >> create_bfdms_dpaas_cluster >> repl_dmdunit_promo_curr_spark_load_job >> repl_dmdunit_promo_curr_upsert_file_archive_in_hdfs >> repl_dmdunit_promo_curr_delete_file_archive_in_hdfs >> repl_dmdunit_promo_curr_delete_oldest_file_upsert >> repl_dmdunit_promo_curr_delete_oldest_file_delete >> repl_dmdunit_promo_curr_delete_signal_file
repl_dmdunit_promo_curr_spark_load_job >> delete_cluster
