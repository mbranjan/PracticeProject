...
SUCCESS_MSG = "repl_algorithm_parm_dly table load was successful"

CLUSTER_NAME = "repl-dl-ephe-repl-algorithm-parm-dly"
...
DAG_DESC = "REPL Algorithm  spark load job"
APP_NAME = "repl_algorithm_parm_dly"
...
repl_algorithm_parm_dly_load_dag = DAG(
    DAG_NAME,
...
SIGNAL_FILE_PATTERN = "Signal_GRS_ALGORITHM_PARM_USWMTNIGHTBATCH*"
POLL_PERIOD = "5"
WAIT_PERIOD = "120"

repl_algorithm_parm_dly_common_event_poll = SSHOperator(
    ...
    dag=repl_algorithm_parm_dly_load_dag,
    trigger_rule='all_success'
)

# **************** Oldest File check Task started ******************
repl_algorithm_parm_dly_oldest_file_check = SSHOperator(
...
OLDEST_FILE = FILE_PATH + "/GRS_ALGORITHM_PARM_USWMTNIGHTBATCH" + DATETIME + "*"
...
COUNTRY_CODE = "us"
SCHEMA_NAME = Variable.get("repl_algorithm_parm_dly_load_dag_var", deserialize_json=True)["schema_name"]
TABLE_NAME = Variable.get("repl_algorithm_parm_dly_load_dag_var", deserialize_json=True)["table_name"]
# SCHEMA_NAME = "ww_repl_dl_secure"
# TABLE_NAME = "repl_algorithm_parm_dly"
# SRCRCVDT = "{{ ti.xcom_pull(task_ids='assign_variable', key='SRCRCVDT') }}"
# SRCRCVTS = "{{ ti.xcom_pull(task_ids='assign_variable', key='SRCRCVTS') }}"
SRCRCVTS = (datetime.now(pytz.timezone('UTC')) - timedelta(1)).strftime("%Y-%m-%d %H:%M:%S")
SRCRCVDT = str(date.today())

LOCATION_US = "gs://d44d65161c7aa338e992de5105bdae5790ad263e56accf675683dbd507f1bd/user/svcmdsedat/landing/incremental/us/" + SCHEMA_NAME + "/" + TABLE_NAME + "/src_rcv_dt"

repl_algorithm_parm_dly_file_move_to_hdfs = SSHOperator(
...
args = ["location=" + LOCATION_US, "src_rcv_dt=" + SRCRCVDT, "schema=" + SCHEMA_NAME, "table=" + TABLE_NAME,
        "src_rcv_ts=" + SRCRCVTS, "bus_dt=" + SRCRCVDT]
...
repl_algorithm_parm_dly_spark_load_job = DCASubmitJobOperator(
    task_id="repl_algorithm_parm_dly_spark_load_job",
    dag=repl_algorithm_parm_dly_load_dag,
    region=REGION,
...
repl_algorithm_parm_dly_file_archive_in_hdfs = BashOperator(
...
repl_algorithm_parm_dly_file_archive_in_hdfs = SSHOperator(
    task_id="repl_algorithm_parm_dly_file_archive_in_hdfs",
...
repl_algorithm_parm_dly_delete_oldest_file = SSHOperator(
...
repl_algorithm_parm_dly_delete_signal_file = SSHOperator(
...
repl_algorithm_parm_dly_common_event_poll >> repl_algorithm_parm_dly_oldest_file_check >> assign_variable >> repl_algorithm_parm_dly_file_move_to_hdfs >> create_bfdms_dpaas_cluster >> repl_algorithm_parm_dly_spark_load_job >> repl_algorithm_parm_dly_file_archive_in_hdfs >> repl_algorithm_parm_dly_delete_oldest_file >> repl_algorithm_parm_dly_delete_signal_file
repl_algorithm_parm_dly_spark_load_job >> delete_cluster
