...
SUCCESS_MSG = "REPL_SUGG_ORDER_SAMS table load was successful"

CLUSTER_NAME = "repl-dl-ephe-repl-sugg-order-sams"

REPL_JAR_PATH = "gs://dca-repl-airflow-dag/JARS/"
REPL_FILE_PATH = "gs://dca-repl-airflow-dag/files/"

DAG_NAME = "repl_sugg_order_sams"
DAG_DESC = "REPL Sugg Order Sams spark load job"
APP_NAME = "REPL_SUGG_ORDER_SAMS"
...
repl_sugg_order_sams_load_dag = DAG(
    DAG_NAME,
...
SIGNAL_FILE_PATTERN = "Signal_GRS_RAW_ORDER_*_JSUSRB5U.txt"
POLL_PERIOD = "5"
WAIT_PERIOD = "120"

repl_sugg_order_sams_common_event_poll = SSHOperator(
    ...
    dag=repl_sugg_order_sams_load_dag
)

# **************** Oldest File check Task started ******************

repl_sugg_order_sams_oldest_file_check = SSHOperator(
...
OLDEST_FILE = FILE_PATH + "/GRS_RAW_ORDER_" + DATETIME + "_JSUSRB5U*"
...
SCHEMA_NAME = Variable.get("repl_sugg_order_load_dag_var", deserialize_json=True)["schema_name"]
TABLE_NAME = Variable.get("repl_sugg_order_load_dag_var", deserialize_json=True)["table_name"]
# SCHEMA_NAME = "ww_repl_dl_secure"
# TABLE_NAME = "repl_sugg_order"

TABLE_PATH = TABLE_NAME + "_sams"

repl_sugg_order_sams_file_move_to_hdfs = SSHOperator(
...
GCP_LOCATION = GCP_BUCKET + "/user/svcmdsedat/raw/incremental/us/" + SCHEMA_NAME + "/" + TABLE_PATH + "/"

create_bfdms_dpaas_cluster = BFDMSDataprocCreateClusterOperator(
    task_id='create_bfdms_dpaas_cluster',
    cluster_name=CLUSTER_NAME, dag=repl_sugg_order_sams_load_dag,
...
def create_spark_job(WORKFLOW_NAME):
    return {
        ...
        "spark_job": {
            "jar_file_uris": [REPL_JAR_PATH + "Repl_Grs_Dual_Op_Cd_Module/ReplGrsDualOpCdModule-1.254.jar",
            ...
            "args": ["workflow=" + WORKFLOW_NAME, "runmode=global", "GCP_Location=" + GCP_LOCATION,
                     ...
                     "TABLE_NAME=" + TABLE_NAME, "SCHEMA_NAME=" + SCHEMA_NAME, "partition=5", "run_dq=true",
...
repl_sugg_order_sams_spark_load_job = DataprocSubmitJobOperator(
    task_id='repl_sugg_order_sams_spark_load_job', job=create_spark_job("REPL_SUGG_ORDER_SAMS"),
    gcp_conn_id=CONN_ID, dag=repl_sugg_order_sams_load_dag, trigger_rule='all_success')
...
TRIG_FILE = f"{FILE_PATH}/{GEO_RGN_CD}_{SCHEMA_NAME}_{TABLE_NAME}_samsus_{DATETIME}.trig"
trig_command = "rm  " + TRIG_FILE + "; echo 'Creating Trigger file: '; touch  " + TRIG_FILE + "; chmod 777 " + TRIG_FILE + ";"

repl_sugg_order_sams_create_trig_file = SSHOperator(
    task_id="repl_sugg_order_sams_create_trig_file",
    ssh_hook=sshHook,
    conn_timeout=600, cmd_timeout=600,
...
repl_sugg_order_sams_delete_oldest_file = SSHOperator(
...
repl_sugg_order_sams_delete_signal_file = SSHOperator(
...
repl_sugg_order_sams_common_event_poll >> repl_sugg_order_sams_oldest_file_check >> assign_variable >> repl_sugg_order_sams_file_move_to_hdfs >> create_bfdms_dpaas_cluster >> repl_sugg_order_sams_spark_load_job >> delete_cluster
repl_sugg_order_sams_spark_load_job >> repl_sugg_order_sams_create_trig_file >> repl_sugg_order_sams_delete_oldest_file >> repl_sugg_order_sams_delete_signal_file
